...
class  EdfFile:    
    ...
    '''    
    ###IMPLEMENTATION WITHOUT OPTIMIZATION WHEN READING ROI
    def GetData(self,Index, DataType="",Pos=None,Size=None):
        ...
        if DataType != "": 
            if  (self.Images[Index].DataType=="UnsignedShort") or \
                (self.Images[Index].DataType=="UnsignedInteger") or \
                 (self.Images[Index].DataType=="UnsignedLong"): 
                Data=self.__SetDataType__ (Data,DataType,1)
            else:
                Data=self.__SetDataType__ (Data,DataType,0)
        if (Pos!=None) or (Size != None):
            if self.Images[Index].NumDim==1:
                if Pos==None: Pos=(0,)
                if Size==None: Size=(0,)
                Data=GetRegion(Data,Pos,Size)            
            elif self.Images[Index].NumDim==2:
                if Pos==None: Pos=(0,0)
        ...
        """ Returns numeric array with image data
            ...
            DataType:       The edf type of the array to be returnd
                            ...
                            Default relation between Edf types and NumPy's typecodes:
                                ...
                                FloatValue          f
                                DoubleValue         d
                            Data signal transformations are handled when reading:
                            - unsigned short images into one long, float or
                              double types
                            - unsigned long image into float or double types
        ...
        """ Writes image to the file. 
            ...
            DataType:       The data type to be saved to the file:
                            ...
                            Default: according to Data array typecode:
                                    ...
                                    f:  FloatValue       
                                    d:  DoubleValue
            WriteAsUnsigned:As NumPy doesn't implement unsigned short and unsigned
                            long types, it is required attention when handling
                            images of these types.
                            Setting WriteAsUnsigned to 1 makes this function
                            converting properly:
                            unsigned short image into one long, float or double
                            unsigned long image into float or double
    ...
    def __SetDataType__ (self,Array,DataType,UnsignedFlag=0):
        ...
        if ToEdfType != FromEdfType:
            ...
            if ((UnsignedFlag==1) and \
                (((FromEdfType ==  "s")     and (ToEdfType in ("i" ,"l","f","d"))) or\
                 ((FromEdfType in ("i","l"))and (ToEdfType in ("f","d"))) ) ):
                aux=Numeric.add(Numeric.less(aux,0)*TypeValue[FromEdfType] , aux)
                #Did this way because it raises error when converting from "O" to "s"                
                if aux.typecode()=="O": aux=aux.astype("d")
                aux=aux.astype(ToEdfType)
            return aux
        return Array
...
def GetDefaultNumericType(EdfType):
    ...
    elif EdfType == "UNSIGNEDSHORT":    return "s"
...
if __name__ == "__main__":    
    ...
    exe=EdfFile("images/tst.edf")
    x=EdfFile("images/x.edf")
