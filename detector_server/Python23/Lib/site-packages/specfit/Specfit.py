...
class Specfit:
    ...
    def __init__(self,parent=None,*vars,**kw):
        ...
        if kw.has_key('Sensitivity'):
            ...
            self.fitconfig['Sensitivity']=1.0
    ...
    def setdata(self,*vars,**kw):
        ...
        if sigmay is None:
            self.sigmay0=Numeric.ones(self.ydata0.shape,Numeric.Float)
            self.sigmay=Numeric.ones(self.ydata0.shape,Numeric.Float)
        else:
    ...
    def addtheory(self,*vars,**kw):
        ...
        if kw.has_key('configure'):
            configure=kw['function']
        elif len(varslist) > 4:
            configure=varslist[4]
        else:
            configure=None
        
        self.theorydict[theory]=[function,parameters,estimate,configure]
    ...
    def importfun(self,file):
        ...
        if type(theory) == type([]):
            ...
            error=self.addtheory(theory,function,parameters,estimate)
            if error:
                    #tkMessageBox.showerror('Error',"Problem implementing user theory")
                    badluck=1
        return badluck
    ...
    def bkg_internal(self,pars,x):
        ...
        #                     pars[0],pars[1])
        if self.bkg_internal_oldpars == pars:
    ...
    def mcafit(self,*vars,**kw):
        ...
        peaks=[]
        if npoints > (1.5)*fwhm:
            peaksidx=SpecfitFuns.seek(ysearch,1,npoints,
                                    fwhm,
                                    sensitivity)
            for idx in peaksidx:
        ...
        for region in regions:
            ...
            self.setdata(self.xdata0,self.ydata0,xmin=region[0],xmax=region[1])
            ...
            self.setdata(self.xdata0,self.ydata0)
    ...
    def mcagetareas(self,**kw):
        ...
        for group in groups:
            ...
            ybkg_around=self.fitfunction(noigno,Numeric.take(x,idx))
