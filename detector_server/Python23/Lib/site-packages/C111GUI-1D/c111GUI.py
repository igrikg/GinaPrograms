...
#########################################
	...
	bc = -1
	if( DBG == TRUE):
	    print "Device: varg.new_DeviceFilename: ", varg.new_DeviceFilename
	...
	if ((bc != constg.GFD) and (bc != constg.MHIT) and ( bc != constg.MHIP) ):
...
# ~~~~~~~~~~~
	...
	self.modeTimer = QTimer(self,"modeTimer")
        ...
        self.progressBar = QProgressDialog( "Downloading Virtex, please wait ....", "In progress ....", 10 )

#
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    def ToolsAutoTune1D(self):
        global varg
        ...
        if( sys.platform == "win32"):
            pid = os.spawnl( os.P_WAIT, "C:\\Program Files\\C111TST\\c111magic.exe"  ,  " -x ")
        else:
            pid = os.spawnl( os.P_WAIT, 'c111magic' , '-x')
            
        if( varg.FD == -1):
            try:
                varg.FD = varg.c111.reopen(varg.new_DeviceFilename)
            except:
                QMessageBox.warning(self,"Warning",self.trUtf8("Cannot re-open connection to device"))
                return
        if( DBG == TRUE):
            varg.c111.setLibDebug(1)
            varg.c111.setDriverDebug(varg.FD,4)
        ...
        if( hwtdc[constg.IDX_FULL_RESOLUTION] == 1):
            endcol = endcol/2
    ...
    def setfiledescriptor(self):
        ...
        if( varg.DefaultCfg.GetRealOperationMode() == constg.GFD):
            varg.cfg_path += constg.CFG_DIR_GFD1D
        elif( varg.DefaultCfg.GetRealOperationMode() == constg.MHIT):
            varg.cfg_path += constg.CFG_DIR_MHIT
        elif( varg.DefaultCfg.GetRealOperationMode() == constg.MHIP):
            varg.cfg_path += constg.CFG_DIR_MHIP
        else:
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~
    def SetUpMenuWindow(self):
        ...
        self.Mode = QPopupMenu(self)
        self.Mode.insertItem( "GFD", self.ModeGFD,0,21)
        self.Mode.insertItem( "MHIT", self.ModeMHIT,0,22)
        self.Mode.insertItem( "MHIP", self.ModeMHIP,0,23)
        self.Mode.connectItem(21,self.Update_window)
        self.Mode.connectItem(22,self.Update_window)
        self.Mode.connectItem(23,self.Update_window)
        ...
        self.Tools.insertItem( "Auto-Tune 1D", self.ToolsAutoTune1D)
        ...
        self.menu.insertItem("Mode",self.Mode)
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~	
    def setUpMainWindow(self):
        ...
        om = varg.DefaultCfg.GetRealOperationMode()
        if (om == constg.GFD or om == constg.MHIT or om ==constg.MHIP ):
            self.C111_mainWidget = QWidget(self,"C111_mainWidget")
            self.C111_MainWindowLayout = QGridLayout(self.C111_mainWidget,1,5,0,10,
                                                            "C111_MainWindowLayout")
            varg.container=Container(self.C111_mainWidget)
            varg.graph = GraphView.GraphView(varg.container,
            ...
            varg.graph.AddMenuPopupItem("Label",self.Label_setting)
            varg.graph.AddMenuSeparator()
            varg.graph.AddMenuPopupItem("Save Image",self.Save_Image1D)
...
#########################################
	...
	if (DBG == TRUE):
	    print "\n TimerDisplay getstatus : ", TDC_getstatus
	    print "\n TimerDisplay START_TDC : ", varg.START_TDC
	...
	varg.graph.SetLabels(varg.Graph_title, varg.Graph_xlabel, varg.Graph_ylabel)
	varg.graph.Update()
	...
	if( varg.FD == -1):
        ...
        fname = QFileDialog.getSaveFileName(
                    directory,
                    "*.data", self, "Save file dialog"
                    "Choose a filename to save under" )
        if fname.isEmpty():
            return
            
        new_DataFilename = fname
        tdcset = varg.CurrentCfg.ToTdcSettings()
        runset = varg.CurrentCfg.ToRunSettings()
        startCol = runset[ constg.IDX_START_COL ]
        endCol = runset[ constg.IDX_END_COL ]
        r = ( endCol - startCol ) 
        if( (r %2) != 0 ):
            r = r - 1
        if( runset[ constg.IDX_BINNING ] == 1 ):
            r = r/2
	
        file_data = open(str(new_DataFilename) + ".data",'w')
        file_data.seek(0)
	if( varg.CurrentCfg.GetRealOperationMode() == constg.GFD ):		
		for i in range( r ):
			file_data.write(str(varg.array_buf_1[i]) + " \n")
	elif (varg.CurrentCfg.GetRealOperationMode() != constg.GFD and
	      runset[ constg.IDX_CHANNELS_NUMBER ] == 1):
		for i in range( r ):
			file_data.write(str(varg.array_buf_1[i]) + " \n")
			#print " idx: ", i , " data: ", varg.array_buf_1[i]			
	elif (runset[ constg.IDX_CHANNELS_NUMBER ] == 2):
		for i in range( r ):
			file_data.write(str(varg.array_buf_1[i])+ "\t" + 
			                        str(varg.array_buf_2[i]) + " \n")
	elif (runset[ constg.IDX_CHANNELS_NUMBER ] == 3):
		for i in range( r ):
			file_data.write(str(varg.array_buf_1[i])+ "\t" + 
			                        str(varg.array_buf_2[i]) + "\t" + 
			                        str(varg.array_buf_3[i]) + " \n")
	elif (runset[ constg.IDX_CHANNELS_NUMBER ] == 4):
		for i in range( r ):
			file_data.write(str(varg.array_buf_1[i])+ "\t" + 
			                        str(varg.array_buf_2[i]) + "\t" + 
			                        str(varg.array_buf_3[i]) + "\t" + 
			                        str(varg.array_buf_4[i]) + " \n")
			                        
	file_data.close()
	self.data_dir = os.path.dirname( str(new_DataFilename) )
	return
	...
	if (DBG == TRUE):
	    print "\n all formats supported : ", varg.graph.GetSaveFormats()
		
	if "PS" not in varg.graph.GetSaveFormats():
            self.statusBar().message('Unable to print',4000)
            return
        if self.printer.setup(self):
            self.statusBar().message('Printing...')
            varg.graph.Save("tmp.ps","PS")
    ...
    def generalHelp(self, *args):
        pid = os.spawnl( os.P_NOWAIT, "C:\Program Files\Internet Explorer\IEXPLORE.EXE"  ,  
                                                       " C:\Python23\Lib\site-packages\C111GUI-1D\C111GuiHelp.html")
        
    def quickStart(self, *args):
                pid = os.spawnl( os.P_NOWAIT, "C:\Program Files\Internet Explorer\IEXPLORE.EXE"  ,  
                                                       " C:\Python23\Lib\site-packages\C111GUI-1D\C111GuiQuickStart.html")
        
    def issuesHelp(self, *args):
                pid = os.spawnl( os.P_NOWAIT, "C:\Program Files\Internet Explorer\IEXPLORE.EXE"  ,  
                                                       " C:\Python23\Lib\site-packages\C111GUI-1D\C111GuiIssues.html")

    def usersGuide(self, *args):
                pid = os.spawnl( os.P_NOWAIT, "C:\Program Files\Internet Explorer\IEXPLORE.EXE"  ,  
                                                       " C:\Python23\Lib\site-packages\C111GUI-1D\C111-TDC-data-sheet.mht")
                                                       
    def serverHelp(self, *args):
                pid = os.spawnl( os.P_NOWAIT, "C:\Program Files\Internet Explorer\IEXPLORE.EXE"  ,  
                                                       " C:\Python23\Lib\site-packages\C111GUI-1D\DasyServerHelp.mht")
...
# ------------------
	...
	ret = QMessageBox.warning( self, "C111 GUI",
            "Are you sure you want to change to Gas-Filled Detector mode.\n\n",
            "Yes",  "No",  None,  0, 1)
        if ret==1:
            return
	...
	if ( operation_mode != constg.GFD ):
		QMessageBox.critical(self,"Critical"," \t Critical Error occured \n Boot configuration for mode GFD has failed")
		varg.c111.close(varg.FD)
		qApp.quit()

        if( ( varg.cfg_path != ( constg.CFG_PATH_MAIN + constg.CFG_DIR_GFD1D ) ) and
            ( varg.cfg_path != ( constg.CFG_PATH_MAIN + constg.CFG_DIR_MHIT ) ) and
            ( varg.cfg_path != ( constg.CFG_PATH_MAIN + constg.CFG_DIR_MHIP ) )         ):
            varg.cfg_path = constg.CFG_PATH_USER + varg.user + "\\" + constg.CFG_DIR_GFD1D
        else:
            varg.cfg_path != constg.CFG_PATH_MAIN + constg.CFG_DIR_GFD1D
            
        tdcset = varg.c111.getTdcConfig(varg.FD)
        varg.CurrentCfg.FromTdcSettings( tdcset )
        varg.CurrentCfg.SetCfgDirPath( varg.cfg_path )
        self.setUpMainWindow()
    ...
    def ModeMHIT(self):
        ...
        if( varg.CurrentCfg.GetRealOperationMode() == constg.MHIT):
            QMessageBox.warning(self,"Warning"," Already using MHIT mode \n No need to change !!! ")
            return
	...
	if (varg.START_TDC == 1):
		...
		pass
	# load virtex with the MHIT file   
        ret = QMessageBox.warning( self, "C111 GUI",
             "Are you sure you want to change to Multi Hit mode.\n\n",
            "Yes", "No", None, 0, 1 )
        if ret==1:
            return 
        pb = QProgressDialog( "Downloading Virtex, please wait ....", "In progress ....", 10 )
        ...
        pb.show()
        if( sys.platform == "win32"):
            Virtex_mhit = varg.c111.loadVirtex(varg.FD,1,"C:\\Program Files\\C111TST")
        else:
            Virtex_mhit = varg.c111.loadVirtex(varg.FD,1,"/users/iorini/C111_7/src")
            
        operation_mode = varg.c111.getBootConfig(varg.FD)
        pb.close(TRUE)
	if ( operation_mode != 1):
            QMessageBox.critical(self,"Critical"," \t Critical Error occured \n Boot configuration for mode MHIT has failed")
            varg.c111.close(varg.FD)
            qApp.quit()
        ...
        if( ( varg.cfg_path != ( constg.CFG_PATH_MAIN + constg.CFG_DIR_GFD1D ) ) and
            ( varg.cfg_path != ( constg.CFG_PATH_MAIN + constg.CFG_DIR_MHIT ) ) and
            ( varg.cfg_path != ( constg.CFG_PATH_MAIN + constg.CFG_DIR_MHIP ) )         ):
            varg.cfg_path = constg.CFG_PATH_USER + varg.user + "\\" + constg.CFG_DIR_MHIT
        else:
            varg.cfg_path != constg.CFG_PATH_MAIN + constg.CFG_DIR_MHIT
    ...
    def ModeMHIP(self):
        ...
        if( varg.CurrentCfg.GetRealOperationMode() == constg.MHIP):
            QMessageBox.warning(self,"Warning"," Already using MHIP mode \n No need to change !!! ")
            return
        ...
        if (varg.START_TDC == 1):
            ...
            pass
        ret = QMessageBox.warning( self, "C111 GUI",
            "Are you sure you want to change to  Multi Hip mode.\n\n", "Yes", "No",  None,  0, 1)
        if ret==1:
            return 
        pb = QProgressDialog( "Downloading Virtex, please wait ....", "In progress ....", 10 )
        pb.setProgress(3)
        pb.show()
        if( sys.platform == "win32"):
            Virtex_mhit = varg.c111.loadVirtex(varg.FD,constg.MHIP,"C:\Program Files\C111TST")
        else:
            Virtex_mhit = varg.c111.loadVirtex(varg.FD,constg.MHIP,"/users/iorini/C111_7/src")
            
        operation_mode = varg.c111.getBootConfig(varg.FD)
        pb.close(TRUE)
        if ( operation_mode != constg.MHIP):
            QMessageBox.critical(self,"Critical"," \t Critical Error occured \n Boot configuration for mode MHIP has failed")
            varg.c111.close(varg.FD)
            qApp.quit()
        ...
        if( ( varg.cfg_path != ( constg.CFG_PATH_MAIN + constg.CFG_DIR_GFD1D ) ) and
            ( varg.cfg_path != ( constg.CFG_PATH_MAIN + constg.CFG_DIR_MHIT ) ) and
            ( varg.cfg_path != ( constg.CFG_PATH_MAIN + constg.CFG_DIR_MHIP ) )         ):
            varg.cfg_path = constg.CFG_PATH_USER + varg.user + "\\" + constg.CFG_DIR_MHIP
        else:
            varg.cfg_path != constg.CFG_PATH_MAIN + constg.CFG_DIR_MHIP
    ...
    def Update_window(self):	
        ...
        if( varg.FD == -1):
            self.Informationlabel.setText("P/C111 DEVICE NOT ATTACHED")
    ...
    def LastConfig(self):
        ...
        if ( (rm != constg.GFD ) and ( rm != constg.MHIT) and ( rm != constg.MHIP) ):		
            QMessageBox.warning(self,"Warning",
		        " The setting of the TDC parameters need to load a mode GFD or MHIT/P")
            return self

        if( rm  != cm ):
            QMessageBox.warning(self,"Warning"," The virtex mode doesn't correspond to the last one loaded")
            return self
        self.STOPTDC_fc()
        if( varg.statsActive == 1):
            varg.dict_dialgs["TdcStatistics"].CloseStats()
    ...
    def NewConfig (self):
        ...
        hwtdc = varg.c111.getTdcConfigHw( varg.FD )
        varg.NewCfg.FromTdcSettings( hwtdc )
        self.STOPTDC_fc()
        if( varg.statsActive == 1):
            varg.dict_dialgs["TdcStatistics"].CloseStats()
...
# -------------------
	...
	if (int(gettdcconfig[ constg.IDX_INTERRUPT_TIMER]) != 1) :
		QMessageBox.warning(self,"Warning"," Timer interruption hasn't been set on TDC configuration ")
		return self
	if( varg.AcquisitionDATA == 1):
            QMessageBox.warning(self,"Warning",
	...
	if (TDC_getstatus['AcqState'] != 1):
        ...
        varg.statsActive = 1
	dialgTDCstat = C111GUI_TDCstatisticDialog(self, "TDC Parameters configuration", 
	                                                                    FALSE, Qt.WType_Dialog)
	...
	cub_status_dialog = C111GUI_CubStatusDisplay(self, "CUB STATUS", FALSE, Qt.WType_Dialog) 
    ...
    def CheckConfig(self):
        ...
        if( cfg_state != constg.CFG_DOWNLOADED and
            ...
            if( last_tdc[constg.IDX_TIMEOUT] == 0):
                if( opmode == constg.GFD):
                    last_tdc[constg.IDX_TIMEOUT] = constg.GFD_MAX_TIMEOUT;
                else:
                    last_tdc[constg.IDX_TIMEOUT] = constg.MHIT_MAX_TIMEOUT;
...
# -------------------------
    def STARTTDC_fc(self):
        ...
        if (TDC_getstatus['AcqState'] == 1):
            ...
            return self
        varg.CurrentCfg.DownloadToTdc()
        # Initialisation of the memory Bank:
        self.buttonSTARTTDC.setDisabled(TRUE)
        self.buttonSTOPTDC.setDisabled(FALSE)
        self.buttonFREEZETDC.setDisabled(FALSE)
        runset = varg.CurrentCfg.ToRunSettings()
        tdcset = varg.CurrentCfg.ToTdcSettings()
        if( (tdcset[constg.IDX_ACQU_BANK] != 0) and (tdcset[constg.IDX_ACQU_BANK] != 1) ):
            tdcset[constg.IDX_ACQU_BANK] = 0
        varg.CurrentCfg.FromTdcSettings( tdcset )
        varg.CurrentCfg.DownloadToTdc()
        ...
        rdh_1 = [   ( runset[constg.IDX_FIRST_CHAN ]  ),
        ...
        rdh_2 = [   runset[constg.IDX_SECOND_CHAN ]  ,
                        runset[ constg.IDX_START_COL] , runset[ constg.IDX_END_COL] ,
                        runset[ constg.IDX_START_ROW], runset[ constg.IDX_END_ROW],
                        runset[ constg.IDX_BINNING]        ]
                
        rdh_3 = [   runset[constg.IDX_THIRD_CHAN ]  ,
                        runset[ constg.IDX_START_COL] , runset[ constg.IDX_END_COL] ,
                        runset[ constg.IDX_START_ROW], runset[ constg.IDX_END_ROW] ,
                        runset[ constg.IDX_BINNING]       ]
                                
        rdh_4 = [   runset[constg.IDX_FOURTH_CHAN ]  ,
                        runset[ constg.IDX_START_COL] , runset[ constg.IDX_END_COL] ,
                        runset[ constg.IDX_START_ROW], runset[ constg.IDX_END_ROW] ,
                        runset[ constg.IDX_BINNING]       ]
...
# -----------------------------
	...
	varg.graph.LogY = not varg.graph.LogY
        if varg.graph.LogY:
            varg.graph.SetYScaleLog()
        else:
            varg.graph.SetYScaleLinear()
        varg.graph.Update()

    def Label_setting(self):
	dialgDisplayLabel = C111GUI_DisplayLabelDialog(self, "Graph Label Setting", TRUE, Qt.WType_Dialog)
	...
	if ( tdcset[ constg.IDX_OPERATION_MODE ] == constg.GFD):
	    if( tdcset[ constg.IDX_PILEUPY] == 0):
	        if (DBG == TRUE):
	            print "\nGFD 1D case in TDC_GraphDisplay() "
                data_buf_1D = Data(None,{})
                data_buf_1D.SetSource(varg.array_buf_1)
                data_buf_1D.LoadSource()
                filter_data_buf_1D = Filter(data_buf_1D,TRUE,TRUE)
                name_1D = "GFD 1D, channel X1/X2"
                p = Pen((0,0,255),3,"solid")
                d = DataSelection.OrthoLineSelection(filter_data_buf_1D.GetData())
                s = GraphView.GraphFilter(name_1D, d,  pen = p )
                varg.graph.SetSource( s )
                varg.graph.Update()
                varg.container.show()

	if( tdcset[ constg.IDX_OPERATION_MODE ] == constg.MHIT or 
	   tdcset[ constg.IDX_OPERATION_MODE ] == constg.MHIP       ):
            data_buf_1 = Data(None,{}) 
            data_buf_1.SetSource(varg.array_buf_1)
            data_buf_1.LoadSource()
            filter_data_buf_1 = Filter(data_buf_1,TRUE,TRUE)
            
            name_1 = "Channel 1 " #+ str( runset[ constg.IDX_FIRST_CHAN] )
            name_2 = "Channel 2 " #+ str( runset[ constg.IDX_SECOND_CHAN] )
            name_3 = "Channel 3 "  #+ str( runset[ constg.IDX_THIRD_CHAN] )
            name_4 = "Channel 4 "  #+ str( runset[ constg.IDX_THIRD_CHAN] )
            if (DBG == TRUE):
                print "\nTDC_GraphDisplay: MHIT/P Nb channels: ", runset[ constg.IDX_CHANNELS_NUMBER]
            if ( runset[ constg.IDX_CHANNELS_NUMBER] == 1):
                #print " 1 channel to plot !!!"
                varg.graph.SetSource(GraphView.GraphFilter(name_1,
                        DataSelection.OrthoLineSelection(filter_data_buf_1.GetData()),pen = Pen((0,0,255),3,"solid")))
                #varg.graph.Update()
            elif ( runset[ constg.IDX_CHANNELS_NUMBER] == 2):
                #print " 2 channel to plot !!!"
                data_buf_2 = Data(None,{}) 
                data_buf_2.SetSource(varg.array_buf_2)
                data_buf_2.LoadSource()
                filter_data_buf_2 = Filter(data_buf_2,TRUE,TRUE)
                varg.graph.SetSource((GraphView.GraphFilter(name_1,
                        DataSelection.OrthoLineSelection(filter_data_buf_1.GetData()),pen = Pen((0,0,255),3,"solid")),
                		GraphView.GraphFilter(name_2,
                		DataSelection.OrthoLineSelection(filter_data_buf_2.GetData()),pen = Pen((0,255,0),3,"solid"))))
                #varg.graph.Update()
            elif (runset[ constg.IDX_CHANNELS_NUMBER] == 3):
                #print " 3 channel to plot !!!"
                data_buf_2 = Data(None,{}) 
                data_buf_2.SetSource(varg.array_buf_2)
                data_buf_2.LoadSource()
                data_buf_3 = Data(None,{})
                data_buf_3.SetSource(varg.array_buf_3)
                data_buf_3.LoadSource()
                filter_data_buf_2 = Filter(data_buf_2,TRUE,TRUE)
                filter_data_buf_3 = Filter(data_buf_3,TRUE,TRUE)
                varg.graph.SetSource((GraphView.GraphFilter(name_1,
                    DataSelection.OrthoLineSelection(filter_data_buf_1.GetData()),pen = Pen((0,0,255),3,"solid")),
                		GraphView.GraphFilter(name_2,
                		    DataSelection.OrthoLineSelection(filter_data_buf_2.GetData()),pen = Pen((0,255,0),3,"solid")),
                		GraphView.GraphFilter(name_3,
                		    DataSelection.OrthoLineSelection(filter_data_buf_3.GetData()),pen = Pen((255,0,0),3,"solid"))))
                #varg.graph.Update()
            elif ( runset[ constg.IDX_CHANNELS_NUMBER] == 4):
                #print " 4 channel to plot !!!"
                data_buf_2 = Data(None,{}) 
                data_buf_2.SetSource(varg.array_buf_2)
                data_buf_2.LoadSource()
                data_buf_3 = Data(None,{})
                data_buf_3.SetSource(varg.array_buf_3)
                data_buf_3.LoadSource()
                data_buf_4 = Data(None,{})
                data_buf_4.SetSource(varg.array_buf_4)
                data_buf_4.LoadSource()
                filter_data_buf_2 = Filter(data_buf_2,TRUE,TRUE)
                filter_data_buf_3 = Filter(data_buf_3,TRUE,TRUE)
                filter_data_buf_4 = Filter(data_buf_4,TRUE,TRUE)
                varg.graph.SetSource((GraphView.GraphFilter(name_1,
                        DataSelection.OrthoLineSelection(filter_data_buf_1.GetData()),pen = Pen((0,0,255),3,"solid")),
                		GraphView.GraphFilter(name_2,
                		    DataSelection.OrthoLineSelection(filter_data_buf_2.GetData()),pen = Pen((0,255,0),3,"solid")),
                		GraphView.GraphFilter(name_3,
                		    DataSelection.OrthoLineSelection(filter_data_buf_3.GetData()),pen = Pen((255,0,0),3,"solid")),
                		GraphView.GraphFilter(name_4,
                		    DataSelection.OrthoLineSelection(filter_data_buf_4.GetData()),pen = Pen((0,0,0),3,"solid"))))
	...
	if (varg.START_TDC == 0):
            if (DBG == TRUE):
                    print "refresh_display: START_TDC==0 "
            pass
        ...
        if(DBG == TRUE):
            print "Refresh_display: TDC config: " , config
        
        BankAcq = config[ constg.IDX_ACQU_BANK ]
        if( BankAcq == 0 ):
            varg.c111.setAcqBank(varg.FD, 0)
            config[ constg.IDX_ACQU_BANK ] = 0;
            varg.array_buf_1_Bank1 = varg.c111.readHistogram(varg.FD, config,varg.rdh[0])
            if ( config[ constg.IDX_OPERATION_MODE] == constg.MHIT or
                 config[ constg.IDX_OPERATION_MODE] == constg.MHIP   ):
                if (DBG == TRUE):
                    print "MHIT/MHIP mode"
                varg.array_buf_2_Bank1 = varg.c111.readHistogram(varg.FD,config,varg.rdh[1])
                varg.array_buf_3_Bank1 = varg.c111.readHistogram(varg.FD,config,varg.rdh[2])
                varg.array_buf_4_Bank1 = varg.c111.readHistogram(varg.FD,config,varg.rdh[3])
            
            varg.c111.setAcqBank(varg.FD, 1)
            config[ constg.IDX_ACQU_BANK ] = 1
            
            varg.array_buf_1_Bank0 = varg.c111.readHistogram(varg.FD,config,varg.rdh[0])
            if ( config[ constg.IDX_OPERATION_MODE] == constg.MHIT or
                 config[ constg.IDX_OPERATION_MODE] == constg.MHIP   ):
                if (DBG == TRUE):
                    print "MHIT/MHIP mode"
                varg.array_buf_2_Bank0 = varg.c111.readHistogram(varg.FD,config,varg.rdh[1])
                varg.array_buf_3_Bank0 = varg.c111.readHistogram(varg.FD,config,varg.rdh[2])
                varg.array_buf_4_Bank0 = varg.c111.readHistogram(varg.FD, config,varg.rdh[3])
        ...
        if ( config[ constg.IDX_OPERATION_MODE] == 1):
            if (DBG == TRUE): print "MHIT mode"
            varg.array_buf_2 = (varg.array_buf_2_Bank0 + varg.array_buf_2_Bank1)
            varg.array_buf_3 = (varg.array_buf_3_Bank0 + varg.array_buf_3_Bank1)
            varg.array_buf_4 = (varg.array_buf_4_Bank0 + varg.array_buf_4_Bank1)  
                     
        self.showMemReadTime()  
        self.TDC_GraphDisplay()

    def Save_Image1D(self):
        global varg
        ...
        if   (Ind_type == ".ps"):
            varg.graph.Save(varg.new_ImageFilename + ".ps","PS")
        elif (Ind_type == ".png"):
            varg.graph.Save(varg.new_ImageFilename + ".png","PNG")
        elif (Ind_type == ".jpg"):
            varg.graph.Save(varg.new_ImageFilename + ".jpg","JPG")
        elif (Ind_type == ".bmp"):
            varg.graph.Save(varg.new_ImageFilename + ".bmp","BMP")
            
        self.data_dir = os.path.dirname( str(varg.new_ImageFilename )  )
...
class C111GUI_TDCstatisticDialog(QDialog):
    def __init__(self,parent = None,name = None,modal = 0,fl = 0):
        ...
        self.ElapsedTimeLineEdit.setAlignment(QLineEdit.AlignLeft)
        ...
        self.ChannelZeroLineEdit.setAlignment(QLineEdit.AlignLeft)
        ...
        self.ChannelOneLineEdit.setAlignment(QLineEdit.AlignLeft)
        ...
        self.ChannelTwoLineEdit.setAlignment(QLineEdit.AlignLeft)
        ...
        self.ChannelThreeLineEdit.setAlignment(QLineEdit.AlignLeft)
        ...
        self.CommonStartsLineEdit.setAlignment(QLineEdit.AlignLeft)
        ...
        self.RejectedEventsLineEdit.setAlignment(QLineEdit.AlignLeft)
        ...
        self.StartsRejectedLineEdit.setAlignment(QLineEdit.AlignLeft)
        ...
        self.OverflowEventsLineEdit.setAlignment(QLineEdit.AlignLeft)
    ...
    def languageChange(self):
        ...
        self.StartsRejectedTextLabel.setText(self.__tr("   Starts / Rejected ( % )  :  "))
...
# ----------------
	...
	self.table_cubstatus.horizontalHeader().setLabel(0," ")
        ...
        #self.table_cubstatus.verticalHeader().setLabel(6,"Low Voltage (general)")
